...
function handleReadNotice(id: string) {
  NoticeAPI.getDetail(id).then((data) => {
    ...
    // 标记为已读（后端持久化），成功后更新当前表格数据
    NoticeAPI.read(id)
      .then(() => {
        const idx = pageData.value.findIndex((n) => n.id === id);
        if (idx >= 0) {
          // 如果当前查询是仅展示未读(isRead === 0)，则移除该行；否则标记为已读
          // queryParams 可能包含 isRead 字段
          if ((queryParams as any).isRead === 0) {
            pageData.value.splice(idx, 1);
            total.value = Math.max(0, total.value - 1);
          } else {
            (pageData.value[idx] as any).isRead = 1;
          }
        }
      })
      .catch(() => {
