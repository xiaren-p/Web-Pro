...
function open(id?: string | number) {
  visible.value = true;
  if (id) {
    title.value = "修改 卖家精灵账号配置";
    SellerAPI.getFormData(String(id)).then((data) => Object.assign(formData, data));
  } else {
    title.value = "新增 卖家精灵账号配置";
...
function handleSubmit() {
  formRef.value.validate((valid: boolean) => {
    if (valid) {
      ...
      if (id) {
        SellerAPI.update(String(id), formData)
          .then(() => {
