...
/**
 * 多选下拉框变更处理：识别"全选"标记并切换全选/取消。
 *
 * @param values - 当前选中的值列表（含全选标记）
 * @param options - 下拉选项列表
 * @param field - 表单字段名（"shops" | "categories" | "managers" | "tags"）
 */
function onSelectChange(
  ...
  const hasSelectAll = values.includes(SELECT_ALL_MARKER);
  const prev = form[field] as (string | number)[];
  const prevHadAll = prev.length === options.length;

  if (hasSelectAll) {
    // 点击全选 → 切换：全选变取消，未全选变全选
    (form[field] as (string | number)[]) = prevHadAll ? [] : options.map((o) => o.value);
  }
  // 否则由 v-model 正常更新
}
...
/** 店铺选项过滤后的列表（用于 el-select 渲染） */
const filteredShopOptions = ref<TimePricingOption[]>([]);

/**
 * 自定义店铺下拉过滤方法：匹配店铺名称或 profile_id。
 * 始终保留"全选"选项在列表顶部。
 *
...
function filterShopOptions(query: string): void {
  ...
  if (!q) {
    ...
    filteredShopOptions.value = shopOptions.value.filter(
      (opt) => String(opt.label).toLowerCase().includes(q) || String(opt.value).includes(q)
    );
  }
}

/** 下拉关闭或清空搜索时重置过滤列表为全部选项 */
