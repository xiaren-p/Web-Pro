    ...
    <div class="data-table-container">
      ...
      >
        ...
        <el-table-column label="分析" width="64" fixed="right" align="center" :resizable="false">
          <template #default="{ row }">
            <template v-if="row.keyword_id && !row._isSummary">
...
/**
 * 打开关键词的调整历史弹窗。
 * 传递 keyword_id + profile_id，查询竞价/分时/暂停记录。
 *
 * @param row - 表格行数据
 */
function openHistory(row: Record<string, unknown>): void {
  adjHistoryDialog.value?.open({
    keyword_id: (row.keyword_id as string) || (row.id as string) || "",
...
const _savedColumns = useLocalStorage<any[]>("keyword_panel_columns", []);
