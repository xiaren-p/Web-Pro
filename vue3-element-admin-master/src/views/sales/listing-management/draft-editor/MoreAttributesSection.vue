        ...
        <div v-loading="loading" class="draft-section__body">
          ...
          <template v-else>
            <div
              ...
              :key="`site-${group.groupKey || '__other__'}`"
              class="more-attrs__field-group"
            >
              <div v-if="group.title" class="more-attrs__field-group-title">
                {{ group.title }}
              </div>
      ...
      <el-col :span="12">
        ...
        <div v-loading="loading" class="draft-section__body">
          ...
          <template v-else>
            <div
              v-for="group in filteredGroupedFields"
              :key="`cn-${group.groupKey || '__other__'}`"
              class="more-attrs__field-group"
            >
              <div v-if="group.title" class="more-attrs__field-group-title">
                {{ group.title }}
 ...
 * 3. groupedOtherFields（按 propertyGroups 分组）动态渲染
 * 4. 工具栏：仅必填过滤 + 字段名搜索
...
const { loading, otherFields, groupedOtherFields, dynamicFormData } = useProductTypeSchema(
...
const filteredGroupedFields = computed<GroupedFieldSection[]>(() => {
  ...
  if (onlyShowRequired.value) {
    groups = groups
      .map((g) => ({
        ...g,
        fields: g.fields.filter((field) => field.required),
      }))
      .filter((g) => g.fields.length);
  }
