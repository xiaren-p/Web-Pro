...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
  ...
  return {
    ...
    server: {
      host: "0.0.0.0",
      // 使用环境变量 `VITE_APP_PORT`，如果未设置则回退到 5173
      port: Number(env.VITE_APP_PORT) || 5173,
      open: true,
      // 统一 /dev-api 代理：避免当 VITE_APP_API_URL 自带 /api/v1 时媒体路径被错误前缀化
      proxy: (() => {
        const rawApiUrl = env.VITE_APP_API_URL || "http://localhost:8000";
        // 拆解 URL，确保 target 只保留协议+主机+端口，不含路径前缀
        let origin: string;
        let apiPath: string = "";
        try {
          const u = new URL(rawApiUrl);
          origin = u.origin; // http(s)://host:port
          apiPath = u.pathname.replace(/\/+$/, ""); // 可能是 '' 或 '/api/v1'
        } catch {
          // 非法格式兜底：直接原样使用；不建议包含路径
          origin = rawApiUrl.replace(/\/api\/v1\/?$/, "");
          apiPath = /\/api\/v1\/?$/.test(rawApiUrl) ? "/api/v1" : "";
        }
        const hasApiV1 = apiPath === "/api/v1";

        return {
          "/dev-api": {
            target: origin, // 永远不带 /api/v1，避免媒体拼接成 /api/v1/media
            changeOrigin: true,
            rewrite: (path: string) => {
              // 媒体静态资源 => /media/*
              ...
              if (hasApiV1) {
                ...
                return path.replace(/^\/dev-api/, apiPath);
              }
              // 没有声明则自动补 /api/v1
              return path.replace(/^\/dev-api/, "/api/v1");
            },
          },
