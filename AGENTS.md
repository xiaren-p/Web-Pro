...
- **反屎山**（§1.2）：物理文件拆分、卫语句前置拦截（嵌套 ≤ 3 级）、函数力求单一职责（>200 行必须拆，Django view / Celery task 等编排方法除外）、禁硬编码（提常量/枚举/环境变量）。
...
- **简洁优先**（铁律 2）：YAGNI，不加未被要求的功能/抽象/灵活性；方法保持单一职责，简洁优先于行数；不创建 < 3 个实现的抽象类。
...
- **卫语句 + 嵌套 ≤ 3**；方法 >200 行抽 `_xxx`（Django view / Celery task 编排方法除外）；禁 `# ======` 长分隔注释，改 `# 主题：说明`。
