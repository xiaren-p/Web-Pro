...
- **单一职责（SRP）**：每个函数/方法力求**单一职责**。**>100 行需审视是否可拆**，**>200 行必须拆分**。Django view / DRF action / Celery task 等框架编排方法天然需要多步骤协调，不做硬性行数约束。拒绝全能上帝类（God Class）。
...
- **圈复杂度削减**：拒绝超长逻辑。方法 >200 行必须抽取为 `私有方法 (_xxx)`。Django view / DRF action / Celery task 等框架编排方法不受此限。严防上帝类 (God Object)。
