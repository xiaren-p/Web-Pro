...
| 模型 | 说明 |
| --- | --- |
| `AiConversation` | 会话表，双 ID（int 主键 + `public_id` UUID），`dify_conversation_id` 维系 Dify 上下文，`group` 外键可空，`pinned_at` 置顶 |
...
- **`DifyClient.stream_chat`**：封装 Dify `/v1/chat-messages` SSE，Bearer 认证、SSE 帧解析、超时（连接 10s / 读 120s），返回 `DifyStreamChunk` 生成器。
...
2. 流式调 Dify，累积 token，每 64 字符阈值落一次库 + 广播 `EVENT_TOKEN`。
...
- **`ai_chat_view.py`**：`POST /ai/chat/` 提交问题；会话列表 / 搜索 / 历史回放 / 重命名 / 置顶 / 删除 / 移动；取消生成（Celery revoke + 广播 done）。
...
- **`ChatPanel.vue`**：右侧 1080px 抽屉主体，左侧栏（新建/搜索/分组/置顶/会话列表）+ 右侧消息流 + 输入区。
...
Pinia store，`panelOpen` / `activeConversationId` 用 `useStorage` 持久化（消息正文不入 localStorage 防敏感数据明文落盘），会话/分组列表缓存，`sending` 锁防重复点击。

### API（`src/api/aiAssistant/aiChat.ts`）

REST 封装（startChat / listConversations / listMessages / delete / rename / pin / cancel / move / search + groups CRUD / reorder），走 `requestV2`；SSE 由 composable 直接 fetch。
...
- 配置：`DIFY_API_BASE` + `DIFY_API_KEY`（`.env`，**仅后端读取**，前端出现 sk- 即严重违规）。
