...
「销售 → 商品图片上传」管理商品图片的上传记录、同步队列与 CSV 批量导入，图片同步队列由内部 `ImageSyncQueue` 模型承载。后端端点前缀 `/api/v1/image-uploads`。
...
- 同步逻辑：后端根据 `ImageUpload` 的 `image_group`（SKU）和 `cloud_path`，在内部 `ImageSyncQueue` 表中 upsert 一条记录（状态置为 `pending`），并向 `ImageUpload.log` 追加日志「已提交同步队列」。
- **注意**：同步操作只追加日志，不修改 `ImageUpload.status` 字段。

## 同步队列

点击「同步队列」→ 弹窗展示内部同步队列记录（`GET /image-uploads/queue`，查询 `ImageSyncQueue` 表）：

- 表格列：序号、图片组（imageGroup）、路径（cloudPath）、状态（pending/success/failed）。
- 支持按图片组后端过滤与后端分页。
...
- 同步操作不更新 `ImageUpload.status`，仅追加 log。
