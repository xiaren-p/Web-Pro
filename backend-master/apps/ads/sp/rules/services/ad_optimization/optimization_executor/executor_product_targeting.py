...
def _check_time_pricing_link(
    ...
    """_check_time_pricing_link（商品投放维度包装）。
...
def _check_all_condition_sets(*args: Any, **kwargs: Any) -> tuple[bool, str]:
    """_check_all_condition_sets（商品投放维度包装）。
...
def _get_last_adjustment_time(
    ...
    """_get_last_adjustment_time（商品投放维度包装）。
...
def _is_execution_cycle_ok(*args: Any, **kwargs: Any) -> tuple[bool, str]:
    """_is_execution_cycle_ok（商品投放维度包装）。
...
def _execute_other_action(
    rule: dict[str, Any], product_target: dict, campaign: LxSpCampaign,
) -> dict[str, Any]:
    """规则级暂停/归档操作 —— 委托 _shared.execute_pause_archive_action。

    商品投放维度：entity_type="product_targeting"，传入 target_id。
    """
    from apps.ads.sp.rules.models.sp_bid_adjustment import PauseEntityTypeChoices

    return execute_pause_archive_action(
        rule,
        campaign_id=campaign.campaign_id,
        profile_id=campaign.profile_id,
        entity_type=PauseEntityTypeChoices.PRODUCT_TARGETING,
        entity_id=product_target["target_id"],
    )
...
def _execute_single_rule(
    ...
    last_time = _get_last_adjustment_time(
        product_target["target_id"], campaign.campaign_id, campaign.profile_id,
    )
    ok, __ = _is_execution_cycle_ok(last_time, cycle)
    if not ok:
        logger.info(
            "[executor_product_targeting] 规则「%s」(%s) pt=%d 执行周期未到",
            rule_name, rule_id, product_target["target_id"],
        )
        return None, False
    ...
    other_result = _execute_other_action(rule, product_target, campaign)
    ...
    if bid_executed:
        ...
        if new_bid is not None:
            apply_bid_update("target", product_target["target_id"], float(new_bid))
        logger.info(
...
def execute_product_targeting_rules() -> dict[str, Any]:
    ...
    # ── 步骤 3：批量加载策略记录 + 广告活动 ──
    unique_pairs = list({(t["campaign_id"], t["profile_id"]) for t in product_targets})
    ...
    for pt in product_targets:
        ...
        rules = _extract_product_targeting_rules(resolve_rules(strategy))
        if not rules:
            continue
    ...
    logger.info(
        ...
        len(product_targets), executed_rule_count,
