...
from apps.ads.sp.rules.services.campaign_adjustment_executor import execute_campaign_adjustment
from api_v2.utils.task_execution_lock import TaskExecutionLock

logger = logging.getLogger(__name__)

# 任务执行锁：视图层调用 is_task_running(LOCK_KEY) 提前判断
LOCK_KEY = "campaign_adjustment_lock"
LOCK_TTL = 960


@shared_task(
    bind=True,
    name="apps.ads.sp.rules.tasks.campaign_adjustment_task.run_campaign_adjustment_task",
    max_retries=0,
    soft_time_limit=840,
    time_limit=900,
    acks_late=True,
...
def run_campaign_adjustment_task(self) -> dict:
    ...
    with TaskExecutionLock(LOCK_KEY, ttl=LOCK_TTL) as acquired:
        if not acquired:
            logger.warning("[run_campaign_adjustment_task] 任务已在执行中，跳过")
