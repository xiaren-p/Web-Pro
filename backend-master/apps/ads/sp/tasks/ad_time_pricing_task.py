"""广告分时策略命中任务（ad_time_pricing_task）。

Celery 定时 / 手动触发调用，扫描新广告并匹配分时策略规则。

注意：该任务必须串行执行（single_thread_queue，concurrency=1），
      防止并发写入 AdTimePricingHit 导致重复记录。
"""
import logging

from celery import shared_task

from apps.ads.sp.services.ad_rules.ad_time_pricing_service import process_new_ads
from api_v2.utils.task_execution_lock import TaskExecutionLock

logger = logging.getLogger(__name__)

# 任务执行锁：视图层调用 is_task_running(LOCK_KEY) 提前判断
# 视图与任务必须共用同一 key，否则互斥语义失效
LOCK_KEY = "ad_time_pricing_lock"
# 锁 TTL 必须 ≥ 任务 time_limit + 缓冲，否则被 SIGKILL 后锁残留时长不够
LOCK_TTL = 1860


@shared_task(
    bind=True,
    name="apps.ads.sp.tasks.ad_time_pricing_task.run_ad_time_pricing_task",
    max_retries=0,
    soft_time_limit=1620,
    time_limit=1800,
    acks_late=True,
)
def run_ad_time_pricing_task(self) -> dict:
    """扫描新广告并匹配分时策略规则。

    由 Celery Beat 定时调用或通过 API 手动触发。
    运行在 single_thread_queue（concurrency=1），Celery 层面保证不并行；
    通过 ``TaskExecutionLock`` 跨入口互斥兜底，配合视图层 ``is_task_running`` 实现
    "API 调度时若有同类任务在跑则返回 409" 的语义。

    Returns:
        dict: {"total_campaigns", "new_ads_processed", "hits", "errors"}
    """
    with TaskExecutionLock(LOCK_KEY, ttl=LOCK_TTL) as acquired:
        if not acquired:
            logger.warning("[run_ad_time_pricing_task] 任务已在执行中，跳过本次调度")
            return {
                "total_campaigns": 0,
                "new_ads_processed": 0,
                "hits": 0,
                "errors": ["任务已在执行中"],
            }

        logger.info("[run_ad_time_pricing_task] 开始扫描新广告并匹配分时策略")
        result = process_new_ads()
        logger.info(
            "[run_ad_time_pricing_task] 完成: total=%d, new=%d, hits=%d, errors=%d",
            result["total_campaigns"],
            result["new_ads_processed"],
            result["hits"],
            len(result["errors"]),
        )
        return result
