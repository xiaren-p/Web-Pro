"""SP 广告优化策略——关键词维度执行器（executor_keyword）。

对 auto_rules 中 comparison_target="keyword" 的规则执行。
数据来源：LxSpKeyword（manual 广告的关键词表）
报表来源：LxSpKeywordReport
"""
...
def _report(kid: int, pid: int, days: int, today: date) -> dict | None:
    ds = today - timedelta(days=days)
    a = LxSpKeywordReport.objects.filter(keyword_id=kid, profile_id=pid, report_date__gte=ds, report_date__lte=today).aggregate(
...
def _time_link(rule: dict, cid: int, pid: int) -> bool:
    ...
    except Exception as e: logger.warning("[kw] 分时查询失败: %s", e, exc_info=True); return False
...
# ── 执行周期 ──
def _last_time(kid: int, cid: int, pid: int) -> datetime | None:
    r = (SpBidAdjustment.objects.filter(
        campaign_id=cid, profile_id=pid, keyword_id=kid,
...
# ── 投放竞价 ──
def _tba(r: dict, kw: LxSpKeyword, camp: LxSpCampaign, today: date) -> tuple[list[dict], bool]:
    ...
    metrics = _report(kw.keyword_id, camp.profile_id, md, today)
    ...
    for i, tb in enumerate(tba_list):
        ...
        if not unlim and "keyword" not in gs: results.append({"序号": i, "状态": "跳过", "原因": "投放对象未选关键词"}); continue
        ...
        for cs in (tb.get("conditionSets") or []):
            ...
            m = metrics if d == md else _report(kw.keyword_id, camp.profile_id, d, today)
            if not m or not _eval_cs(cs, m): cs_ok = False; cs_fail = f"投放级条件组（近{d}天）未通过"; break
        if not cs_ok: results.append({"序号": i, "状态": "跳过", "原因": cs_fail, "关键词": f"{kw.keyword_id}({kw.keyword_text})", "报表": metrics}); continue

        cur = float(kw.bid) if kw.bid else 0
        nb = _bid(cur, ba)
        if nb is None: results.append({"序号": i, "状态": "跳过", "原因": f"不支持: {bt}"}); continue
        if nb == cur: results.append({"序号": i, "状态": "无需调整", "关键词": kw.keyword_text, "竞价": cur}); continue

        executed = True
        results.append({"序号": i, "状态": "待执行", "关键词ID": kw.keyword_id, "关键词": kw.keyword_text,
...
# ── 单条规则 ──
def _rule(rule: dict, kw: LxSpKeyword, camp: LxSpCampaign, today: date) -> tuple[dict | None, bool]:
    ...
    m = _report(kw.keyword_id, camp.profile_id, md, today)
    ok, reason = _check_all(rule, m)
    if not ok: logger.info("[kw] rule「%s」kw=%d 条件不通过: %s", rname, kw.keyword_id, reason); return None, False
    tb_results, bid_done = _tba(rule, kw, camp, today)
    res = {"规则ID": rid, "规则名称": rname, "优先级": rule.get("priority", 0),
           "广告活动ID": camp.campaign_id, "店铺ID": camp.profile_id,
           "关键词ID": kw.keyword_id, "关键词": kw.keyword_text,
           "当前竞价": float(kw.bid) if kw.bid else 0, "报表": m,
           "预算": _budget(rule), "其他": _other(rule), "投放竞价": tb_results}
    _debug(f"kw_{kw.keyword_id}_{rid}.json", res); return res, bid_done

# ── 提取 ──
def _extract(ar: list) -> list[dict]:
    rules = []
    for it in ar:
        if isinstance(it, dict) and it.get("comparison_target") == "keyword": rules.extend(it.get("rules") or [])
    rules.sort(key=lambda r: r.get("priority", 0)); return rules

# ═══════════════ 主入口 ═══════════════
def execute_keyword_rules() -> dict[str, Any]:
    """
    优化策略——关键词维度执行。前置周期跳过 + 竞价全局互斥。
    """
    today = date.today(); _ensure()

    # ── 1. 扫描 ──
    kws = list(LxSpKeyword.objects.filter(state="enabled", bid__isnull=False)
               .only("campaign_id", "profile_id", "keyword_id", "keyword_text", "bid", "ad_group_id"))
    if not kws: return {"扫描": 0, "周期跳过": 0, "有策略": 0, "执行规则": 0, "受影响": 0, "详情": [], "错误": []}
    logger.info("[kw] 扫描: %d", len(kws))

    # ── 2. 前置周期跳过 ──
    cycle_skip = 0; active = []
    for kw in kws:
        lt = _last_time(kw.keyword_id, kw.campaign_id, kw.profile_id)
        ok, _ = _cycle_ok(lt, _DEFAULT_CYCLE_DAYS)
        if not ok: cycle_skip += 1
        else: active.append(kw)
    logger.info("[kw] 周期跳过: %d, 进入: %d", cycle_skip, len(active))
    if not active: return {"扫描": len(kws), "周期跳过": cycle_skip, "有策略": 0, "执行规则": 0, "受影响": 0, "详情": [], "错误": []}

    # ── 3. 批量加载 ──
    pairs = list({(k.campaign_id, k.profile_id) for k in active})
    ...
    logger.info("[kw] 有策略: %d", len(sm))

    # ── 4. 遍历执行 ──
    results, errors, executed, affected = [], [], 0, set()
    for kw in active:
        camp = cm.get((kw.campaign_id, kw.profile_id))
        if not camp: continue
        strat = sm.get((kw.campaign_id, kw.profile_id, "manual"))
        ...
        for rule in rules:
            if bid_already_done:
                logger.info("[kw] kw=%d 已有竞价, 跳过规则「%s」", kw.keyword_id, rule.get("rule_name", "?")); continue
            try:
                res, bid_done = _rule(rule, kw, camp, today)
            except Exception as e:
                msg = f"kw={kw.keyword_id} rule={rule.get('rule_id')} {e}"; logger.exception("[kw] %s", msg); errors.append(msg); continue
            if res is not None:
                results.append(res); executed += 1; affected.add(kw.keyword_id)
                if bid_done: bid_already_done = True
                break

    logger.info("[kw] done: active=%d executed=%d affected=%d", len(active), executed, len(affected))
    return {"扫描": len(kws), "周期跳过": cycle_skip, "有策略": len(sm), "执行规则": executed, "受影响": len(affected), "详情": results, "错误": errors}
